...
def readColmapCameras(cam_extrinsics, cam_intrinsics, depths_params, images_folder, depths_folder, masks_folder):
    cam_infos = []
    mask_count = 0
    rgb_count = 0
    depth_count = 0
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, depth_params=depth_params, image=image, mask=mask,
                              image_path=image_path, depth_path=depth_path, mask_path=mask_path, image_name=image_name,
                              width=width, height=height)
        cam_infos.append(cam_info)

    sys.stdout.write('\n')
    sys.stdout.write("Read {} images\n".format(rgb_count))
    sys.stdout.write("Read {} masks\n".format(mask_count))
    sys.stdout.write("Read {} depths\n".format(depth_count))
    sys.stdout.write('\n')
    return cam_infos
...
def readColmapSceneInfo(path, images, depths, eval, llffhold=8, masks=None):
