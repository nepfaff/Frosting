...
def get_scales(key, cameras, images, points3d_ordered, args):
    ...
    # Invert depth if it's not already inverse depth
    if not args.is_inverse_depth:
        # Avoid division by zero for background or invalid depth values
        invmonodepthmap = np.where(invmonodepthmap > 0, 1.0 / invmonodepthmap, 0.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_type", default="txt")
    parser.add_argument("--is_inverse_depth", default=False)
