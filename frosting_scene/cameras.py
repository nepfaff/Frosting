...
def load_gs_cameras(
    ...
    if len(error_names_list) > 0:
        print(
            "Warning: Found multiple cameras with same GT image name:",
            error_names_list,
            sep="\n",
        )
    ...
    camera_transforms = sorted(
        unsorted_camera_transforms.copy(), key=lambda x: x["img_name"]
    )
    ...
    for cam_idx in range(len(camera_transforms)):
        ...
        if load_gt_images:
            ...
            resolution = round(orig_w / (downscale_factor)), round(
                orig_h / (downscale_factor)
            )
            ...
            if max(height, width) > max_img_size:
                ...
                downscale_factor = additional_downscale_factor * downscale_factor
            image_height, image_width = round(height / downscale_factor), round(
        ...
        if depths_params is not None:
            try:
                depth_params = depths_params[name]
            except:
                print("\n", name, "not found in depths_params")
        depths_folder = "" if depths_folder is None else depths_folder
        depth_path = (
            os.path.join(depths_folder, name + extension) if depths_folder != "" else ""
        )
        if depth_path != "":
    ...
    def __init__(
        ...
        if image is None:
            if image_height is None or image_width is None:
                raise ValueError(
                    "Either image or image_height and image_width must be specified"
                )
            else:
            ...
            if mask is not None:
                self.is_masked = (mask == 0).expand(
                    *image.shape
                )  # True represent masked pixels
...
def create_p3d_cameras(R=None, T=None, K=None, znear=0.0001):
    ...
    if K is not None:
        ...
        p3d_cameras.K = (
            p3d_cameras.get_projection_transform().get_matrix().transpose(-1, -2)
        )
...
def convert_camera_from_gs_to_pytorch3d(gs_cameras, device="cuda"):
    ...
    fx = torch.Tensor(
        np.array(
            [
                fov2focal(gs_camera.FoVx, gs_camera.image_width)
    ...
    fy = torch.Tensor(
        np.array(
            [
                fov2focal(gs_camera.FoVy, gs_camera.image_height)
                for gs_camera in gs_cameras
            ]
        )
    ).to(device)
    image_height = torch.tensor(
        np.array([gs_camera.image_height for gs_camera in gs_cameras]), dtype=torch.int
    ).to(device)
    image_width = torch.tensor(
        np.array([gs_camera.image_width for gs_camera in gs_cameras]), dtype=torch.int
    ).to(device)
    cx = image_width / 2.0  # torch.zeros_like(fx).to(device)
    cy = image_height / 2.0  # torch.zeros_like(fy).to(device)
    ...
    focal_pytorch3d = torch.Tensor([fx[0], fy[0]])[None].to(device) / scale
...
def convert_camera_from_pytorch3d_to_gs(
    ...
    for cam_idx in range(N):
        ...
        c2w = (
            torch.cat([c2w, torch.Tensor([[0, 0, 0, 1]]).to(device)], dim=0)
            .cpu()
            .numpy()
        )  # .transpose(-1, -2)
...
class CamerasWrapper:
    ...
    def __init__(
        ...
        self.height = torch.tensor(
            np.array([gs_camera.image_height for gs_camera in gs_cameras]),
            dtype=torch.int,
        ).to(device)
        self.width = torch.tensor(
            np.array([gs_camera.image_width for gs_camera in gs_cameras]),
            dtype=torch.int,
        ).to(device)
        self.cx = self.width / 2.0  # torch.zeros_like(fx).to(device)
        self.cy = self.height / 2.0  # torch.zeros_like(fy).to(device)
    ...
    def get_spatial_extent(self):
        ...
        half_diagonal = (
            torch.norm(camera_centers - avg_camera_center, dim=-1).max().item()
        )
