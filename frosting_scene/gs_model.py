...
class GaussianSplattingWrapper:
    ...
    def __init__(
        ...
        if os.path.basename(source_path) in [
            "chair",
            "drums",
            "ficus",
            "hotdog",
            ...
            if len(remove_camera_indices) == 0.0:
        ...
        depth_params_file = os.path.join(
            source_path, "sparse/0", "depth_params.json"
        )
        depths_params = None
        if depth_path != "" and depth_path is not None:
            try:
    ...
    def get_gt_depth(self, camera_indices: int, to_cuda=False):
        """Returns the ground truth depth map corresponding to the training camera at the given index.
        ...
        Returns:
            Tensor: The ground truth depth map.
        """
        gt_depth = self.cam_list[camera_indices].invdepthmap
        if to_cuda:
            gt_depth = gt_depth.cuda()
        return gt_depth.permute(1, 2, 0)

    def get_depth_reliable(self, camera_indices: int) -> bool:
        depth_reliable = self.cam_list[camera_indices].depth_reliable
        return depth_reliable
