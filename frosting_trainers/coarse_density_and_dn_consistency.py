...
def depth_normal_consistency_loss(
    ...
    normal_from_depth = (
        normal_from_depth @ camera.world_view_transform[:3, :3]
    ).permute(2, 0, 1)

    if scale_rendered_normals:
        # We normalize the normals to have the same scale as the normals from the depth map.
        normal_view = (
            (normal - normal.mean()) / normal.std()
        ) * normal_from_depth.std() + normal_from_depth.mean()
    else:
        normal_view = normal
...
def coarse_training_with_density_regularization_and_dn_consistency(args):
    ...
    if initialize_from_trained_3dgs:
        densify_from_iter = (
            500 + 99999
        )  # 500  # Maybe reduce this, since we have a better initialization?
        densify_until_iter = 7000 - 7000  # 7000
    else:
        densify_from_iter = (
            500  # 500  # Maybe reduce this, since we have a better initialization?
        )
    ...
    if regularize_sdf:
        ...
        if use_sdf_estimation_loss or enforce_samples_to_be_on_surface:
            ...
            close_gaussian_threshold = 2.0  # 2.
        ...
        density_factor = 1.0 / 16.0  # Should be equal to 1. / regularity_knn
        if (
            use_sdf_estimation_loss or enforce_samples_to_be_on_surface
        ) and sdf_estimation_mode == "density":
            density_factor = 1.0
        density_threshold = 1.0  # 0.5 * density_factor
    ...
    if bind_to_surface_mesh:
        ...
        if use_surface_mesh_laplacian_smoothing_loss:
            surface_mesh_laplacian_smoothing_method = (
                "uniform"  # "cotcurv", "cot", "uniform"
            )
            surface_mesh_laplacian_smoothing_factor = 5.0  # 0.1
    ...
    if regularize_sdf:
        ...
        start_reset_neighbors_from = (
            7000 + 1
        )  # 0 until now (should be equal to regularize_from + 1?)
    ...
    if do_resolution_warmup:
        ...
        current_resolution_factor = downscale_resolution_factor * 4.0
    else:
    ...
    if args.output_dir is None:
        if len(args.scene_path.split("/")[-1]) > 0:
            args.output_dir = os.path.join(
                "./output/coarse", args.scene_path.split("/")[-1]
            )
        else:
            args.output_dir = os.path.join(
                "./output/coarse", args.scene_path.split("/")[-2]
            )
    ...
    if depth_path is not None and depth_path != "":
        # Depth path is relative to data folder.
    ...
    sugar_checkpoint_path = (
        f"sugarcoarse_3Dgs{iteration_to_load}_densityestimXX_sdfnormYY/"
    )
    ...
    ).replace("YY", str(sdf_better_normal_factor).replace(".", ""))
    ...
    CONSOLE.print("Depth path:", depth_path)
    CONSOLE.print("   > Content:", len(os.listdir(depth_path)))
    ...
    if initialize_from_trained_3dgs:
        with torch.no_grad():
            ...
            colors = SH2RGB(
                nerfmodel.gaussians.get_features[:, 0].detach().float().cuda()
            )
            if prune_at_start:
                with torch.no_grad():
                    start_prune_mask = (
                        nerfmodel.gaussians.get_opacity.view(-1)
                        > start_pruning_threshold
                    )
    ...
    if bind_to_surface_mesh:
        surface_mesh_to_bind_full_path = os.path.join(
            "./results/meshes/", surface_mesh_to_bind_path
        )
    ...
    if initialize_from_trained_3dgs:
        with torch.no_grad():
            ...
            if prune_at_start:
                ...
                sugar._sh_coordinates_dc[...] = (
                    nerfmodel.gaussians._features_dc.detach()[start_prune_mask]
                )
                sugar._sh_coordinates_rest[...] = (
                    nerfmodel.gaussians._features_rest.detach()[start_prune_mask]
                )
            else:
                sugar._scales[...] = nerfmodel.gaussians._scaling.detach()
                sugar._quaternions[...] = nerfmodel.gaussians._rotation.detach()
                sugar.all_densities[...] = nerfmodel.gaussians._opacity.detach()
                sugar._sh_coordinates_dc[...] = (
                    nerfmodel.gaussians._features_dc.detach()
                )
                sugar._sh_coordinates_rest[...] = (
                    nerfmodel.gaussians._features_rest.detach()
                )

    CONSOLE.print(f"\nSuGaR model has been initialized.")
    CONSOLE.print(sugar)
    CONSOLE.print(
        f"Number of parameters: {sum(p.numel() for p in sugar.parameters() if p.requires_grad)}"
    )
    ...
    if loss_function == "l1":
        ...
        def loss_fn(pred_rgb, gt_rgb):
            return (1.0 - dssim_factor) * l1_loss(pred_rgb, gt_rgb) + dssim_factor * (
                1.0 - ssim(pred_rgb, gt_rgb)
            )
    ...
    depth_l1_weight = get_expon_lr_func(1.0, 0.01, max_steps=num_iterations - iteration)
    ...
    for batch in range(9_999_999):
        ...
        for i in range(0, train_num_images, train_num_images_per_batch):
            ...
            if (
                regularize
                and prune_when_starting_regularization
                and iteration == regularize_from + 1
            ) or ((iteration - 1) in prune_low_opacity_gaussians_at):
                CONSOLE.print(
                    "\nPruning gaussians with low-opacity for further optimization..."
                )
                prune_mask = (
                    gaussian_densifier.model.strengths < prune_hard_opacity_threshold
                ).squeeze()
            ...
            alpha_mask = nerfmodel.get_alpha_mask(
            ...
            if not no_rendering:
                ...
                pred_rgb = outputs["image"].view(
                    -1, sugar.image_height, sugar.image_width, 3
                )
                pred_invdepth = outputs["depth"]
                radii = outputs["radii"]
                ...
                gt_image = nerfmodel.get_gt_image(
                    camera_indices=camera_indices, to_cuda=True
                )
                ...
                if alpha_mask is not None:
                    gt_rgb = gt_rgb * alpha_mask + bg_tensor.view(1, 1, 1, 3) * (
                        1.0 - alpha_mask
                    )
                ...
                pred_rgb = pred_rgb.transpose(-1, -2).transpose(
                    -2, -3
                )  # TODO: Change for torch.permute
                gt_rgb = gt_rgb.transpose(-1, -2).transpose(-2, -3)

                # Compute loss
                loss = loss_fn(pred_rgb, gt_rgb)

                # Set big depth value in the background.
                if alpha_mask is not None:
                    # Inverse depth is 1/d. Hence, 0 means infinite depth.
                    pred_invdepth[alpha_mask == 0] = 0.0

                # Depth regularization.
                Ll1depth_pure = 0.0
                if depth_l1_weight(iteration) > 0 and depth_reliable:
                    # Exclude masked pixels from the loss.
                    Ll1depth_pure = torch.abs(
                        (pred_invdepth - gt_invdepth) * mask
                    ).mean()
                ...
                if (
                    enforce_entropy_regularization
                    and iteration > start_entropy_regularization_from
                    and iteration < end_entropy_regularization_at
                ):
                ...
                # Depth-Normal consistency
                if (
                    enforce_depth_normal_consistency
                    and iteration > start_dn_consistency_from
                ):
                    ...
                    normal_error = depth_normal_consistency_loss(
                        ...
                        normal=normal_img.permute(
                            2, 0, 1
                        ),  # Shape is (3, height, width)
                        camera=nerfmodel.training_cameras.gs_cameras[
                            camera_indices.item()
                        ],
                        scale_rendered_normals=False,
                        return_normal_maps=False,
                ...
                if regularize:
                    ...
                    if iteration > regularize_from:
                        ...
                        if (iteration >= start_reset_neighbors_from) and (
                            (iteration == regularize_from + 1)
                            or (iteration % reset_neighbors_every == 0)
                        ):
                            CONSOLE.print("\n---INFO---\nResetting neighbors...")
                            sugar.reset_neighbors()
                        neighbor_idx = sugar.get_neighbors_of_random_points(
                            num_samples=regularity_samples,
                        ...
                        if regularize_sdf and iteration > start_sdf_regularization_from:
                            ...
                            if (
                                use_sdf_estimation_loss
                                or enforce_samples_to_be_on_surface
                            ) and iteration > start_sdf_estimation_from:
                                if iteration == start_sdf_estimation_from + 1:
                                    CONSOLE.print(
                                        "\n---INFO---\nStarting SDF estimation loss."
                                    )
                                fov_camera = nerfmodel.training_cameras.p3d_cameras[
                                    camera_indices.item()
                                ]
                                ...
                                if use_projection_as_estimation:
                                    ...
                                    if backpropagate_gradients_through_depth:
                                        point_depth = (
                                            fov_camera.get_world_to_view_transform()
                                            .transform_points(sugar.points)[..., 2:]
                                            .expand(-1, 3)
                                        )
                                ...
                                if sample_only_in_gaussians_close_to_surface:
                                    with torch.no_grad():
                                        ...
                                        gaussian_centers_z = (
                                            gaussian_centers_in_camera_space[..., 2]
                                            + 0.0
                                        )
                                        gaussian_centers_map_z = (
                                            sugar.get_points_depth_in_depth_map(
                                                fov_camera,
                                                depth,
                                        ...
                                        gaussian_standard_deviations = (
                                            sugar.scaling
                                            * quaternion_apply(
                                                quaternion_invert(sugar.quaternions),
                                                gaussian_to_camera,
                                            )
                                        ).norm(dim=-1)

                                        gaussians_close_to_surface = (
                                            gaussian_centers_map_z - gaussian_centers_z
                                        ).abs() < close_gaussian_threshold * gaussian_standard_deviations
                                        sampling_mask = (
                                            sampling_mask * gaussians_close_to_surface
                                        )

                            n_gaussians_in_sampling = sampling_mask.sum()
                            if n_gaussians_in_sampling > 0:
                                sdf_samples, sdf_gaussian_idx = (
                                    sugar.sample_points_in_gaussians(
                                ...
                                if (
                                    ...
                                    if use_projection_as_estimation:
                                        proj_mask = torch.ones_like(
                                            sdf_samples[..., 0], dtype=torch.bool
                                        )
                                        samples_gaussian_normals = sugar.get_normals(
                                            estimate_from_points=False
                                        )[sdf_gaussian_idx]
                                        sdf_estimation = (
                                            (
                                                sdf_samples
                                                - sugar.points[sdf_gaussian_idx]
                                            )
                                            * samples_gaussian_normals
                                        ).sum(
                                            dim=-1
                                        )  # Shape is (n_samples,)
                                    else:
                                        ...
                                        sdf_samples_z = (
                                            sdf_samples_in_camera_space[..., 2] + 0.0
                                        )
                                        ...
                                        sdf_samples_map_z = (
                                            sugar.get_points_depth_in_depth_map(
                                                fov_camera,
                                                depth,
                                                sdf_samples_in_camera_space[proj_mask],
                                            )
                                        )
                                        sdf_estimation = (
                                            sdf_samples_map_z - sdf_samples_z[proj_mask]
                                        )

                                    if not sample_only_in_gaussians_close_to_surface:
                                        if normalize_by_sdf_std:
                                            print(
                                                "Setting normalize_by_sdf_std to False because sample_only_in_gaussians_close_to_surface is False."
                                            )
                                    ...
                                    with torch.no_grad():
                                        if normalize_by_sdf_std:
                                            ...
                                            sdf_sample_std = (
                                                sugar.get_cameras_spatial_extent()
                                                / 10.0
                                            )
                                    ...
                                    if use_sdf_estimation_loss:
                                        if sdf_estimation_mode == "sdf":
                                            ...
                                            loss = (
                                                loss
                                                + sdf_estimation_factor
                                                * sdf_estimation_loss.clamp(
                                                    max=10.0
                                                    * sugar.get_cameras_spatial_extent()
                                                ).mean()
                                            )
                                            ...
                                            target_densities = torch.exp(
                                                -0.5
                                                * sdf_estimation.pow(2)
                                                / beta.pow(2)
                                            )
                                            if squared_sdf_estimation_loss:
                                                sdf_estimation_loss = (
                                            ...
                                            loss = (
                                                loss
                                                + sdf_estimation_factor
                                                * sdf_estimation_loss.mean()
                                            )
                                        else:
                                            raise ValueError(
                                                f"Unknown sdf_estimation_mode: {sdf_estimation_mode}"
                                            )

                                    if enforce_samples_to_be_on_surface:
                                        if squared_samples_on_surface_loss:
                                            samples_on_surface_loss = (
                                                sdf_estimation / sdf_sample_std
                                            ).pow(2)
                                        else:
                                            samples_on_surface_loss = (
                                                sdf_estimation.abs() / sdf_sample_std
                                            )
                                        loss = (
                                            loss
                                            + samples_on_surface_factor
                                            * samples_on_surface_loss.clamp(
                                                max=10.0
                                                * sugar.get_cameras_spatial_extent()
                                            ).mean()
                                        )

                                if use_sdf_better_normal_loss and (
                                    iteration > start_sdf_better_normal_from
                                ):
                                    if iteration == start_sdf_better_normal_from + 1:
                                        CONSOLE.print(
                                            "\n---INFO---\nStarting SDF better normal loss."
                                        )
                                    closest_gaussians_idx = sugar.knn_idx[
                                        sdf_gaussian_idx
                                    ]
                                    # Compute minimum scaling
                                    closest_min_scaling = (
                                        sugar.scaling.min(dim=-1)[0][
                                            closest_gaussians_idx
                                        ]
                                        .detach()
                                        .view(len(sdf_samples), -1)
                                    )

                                    # Compute normals and flip their sign if needed
                                    closest_gaussian_normals = sugar.get_normals(
                                        estimate_from_points=False
                                    )[closest_gaussians_idx]
                                    samples_gaussian_normals = sugar.get_normals(
                                        estimate_from_points=False
                                    ...
                                    closest_gaussian_opacities = fields[
                                        "closest_gaussian_opacities"
                                    ].detach()  # Shape is (n_samples, n_neighbors)
                                    normal_weights = (
                                        (
                                            (
                                                sdf_samples[:, None]
                                                - sugar.points[closest_gaussians_idx]
                                            )
                                            * closest_gaussian_normals
                                        )
                                        .sum(dim=-1)
                                        .abs()
                                    )  # Shape is (n_samples, n_neighbors)
                                    ...
                                    normal_weights_sum = normal_weights.sum(
                                        dim=-1
                                    ).detach()  # Shape is (n_samples,)
                                    normal_weights = (
                                        normal_weights
                                        / normal_weights_sum.unsqueeze(-1).clamp(
                                            min=1e-6
                                        )
                                    )  # Shape is (n_samples, n_neighbors)
                                    ...
                                    loss = (
                                        loss
                                        + sdf_better_normal_factor
                                        * sdf_better_normal_loss.mean()
                                    )
                            else:
                                CONSOLE.log(
            ...
            if bind_to_surface_mesh:
                ...
                if use_surface_mesh_laplacian_smoothing_loss:
                    loss = (
                        loss
                        + surface_mesh_laplacian_smoothing_factor
                ...
                if use_surface_mesh_normal_consistency_loss:
                    loss = (
                        loss
                        + surface_mesh_normal_consistency_factor
            ...
            with torch.no_grad():
                if (not no_rendering) and (iteration < densify_until_iter):
                    gaussian_densifier.update_densification_stats(
                        viewspace_points, radii, visibility_filter=radii > 0
                    )
                    ...
                    if (
                        ...
                        size_threshold = (
                            gaussian_densifier.max_screen_size
                            if iteration > opacity_reset_interval
                            else None
                        )
                        gaussian_densifier.densify_and_prune(
                            densify_grad_threshold,
                            prune_opacity_threshold,
                            cameras_spatial_extent,
                            size_threshold,
                        )
                        CONSOLE.print(
                            "Gaussians densified and pruned. New number of gaussians:",
                            len(sugar.points),
                        )

                        if (
                            regularize
                            and (iteration > regularize_from)
                            and (iteration >= start_reset_neighbors_from)
                        ):
                            sugar.reset_neighbors()
            ...
            if iteration == 1 or iteration % print_loss_every_n_iterations == 0:
                ...
                CONSOLE.print(
                    f"loss: {loss:>7f}  [{iteration:>5d}/{num_iterations:>5d}]",
                    "computed in",
                    (time.time() - t0) / 60.0,
                    "minutes.",
                )
                with torch.no_grad():
                    ...
                    CONSOLE.print(
                        "Points:",
                        sugar.points.min().item(),
                        sugar.points.max().item(),
                        sugar.points.mean().item(),
                        sugar.points.std().item(),
                        sep="   ",
                    )
                    CONSOLE.print(
                        "Scaling factors:",
                        sugar.scaling.min().item(),
                        sugar.scaling.max().item(),
                        sugar.scaling.mean().item(),
                        sugar.scaling.std().item(),
                        sep="   ",
                    )
                    CONSOLE.print(
                        "Quaternions:",
                        sugar.quaternions.min().item(),
                        sugar.quaternions.max().item(),
                        sugar.quaternions.mean().item(),
                        sugar.quaternions.std().item(),
                        sep="   ",
                    )
                    CONSOLE.print(
                        "Sh coordinates dc:",
                        sugar._sh_coordinates_dc.min().item(),
                        sugar._sh_coordinates_dc.max().item(),
                        sugar._sh_coordinates_dc.mean().item(),
                        sugar._sh_coordinates_dc.std().item(),
                        sep="   ",
                    )
                    CONSOLE.print(
                        "Sh coordinates rest:",
                        sugar._sh_coordinates_rest.min().item(),
                        sugar._sh_coordinates_rest.max().item(),
                        sugar._sh_coordinates_rest.mean().item(),
                        sugar._sh_coordinates_rest.std().item(),
                        sep="   ",
                    )
                    CONSOLE.print(
                        "Opacities:",
                        sugar.strengths.min().item(),
                        sugar.strengths.max().item(),
                        sugar.strengths.mean().item(),
                        sugar.strengths.std().item(),
                        sep="   ",
                    )
                    if regularize_sdf and iteration > start_sdf_regularization_from:
                        CONSOLE.print(
                            "Number of gaussians used for sampling in SDF regularization:",
                            n_gaussians_in_sampling,
                        )
                t0 = time.time()

            # Save model
            if (iteration % save_model_every_n_iterations == 0) or (
                iteration in save_milestones
            ):
            ...
            if (
                do_sh_warmup
                and (iteration > 0)
                and (current_sh_levels < sh_levels)
                and (iteration % sh_warmup_every == 0)
            ):
                current_sh_levels += 1
                CONSOLE.print(
                    "Increasing number of spherical harmonics levels to",
                    current_sh_levels,
                )

            if (
                do_resolution_warmup
                and (iteration > 0)
                and (current_resolution_factor > 1)
                and (iteration % resolution_warmup_every == 0)
            ):
                current_resolution_factor /= 2.0
    ...
    CONSOLE.print(
        f"Training finished after {num_iterations} iterations with loss={loss.detach().item()}."
    )
