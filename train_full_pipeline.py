...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Script to optimize a full Frosting model."
    )

    # Data
    parser.add_argument(
        "-s", "--scene_path", type=str, help="(Required) path to the scene data to use."
    )
    ...
    parser.add_argument(
        "--smooth_initial_frosting",
        type=str2bool,
        default=True,
        help="If True, smooth the initial frosting.",
    )
    parser.add_argument(
        "--n_neighbors_for_smoothing",
        type=int,
        default=4,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the masks to use for the scene.",
    )
    parser.add_argument(
        "--depths", type=str, default=None, help="Path to the depth maps."
    )
    ...
    if args.export_obj:
        print(
            "Will export a textured mesh as an obj file for visualization and edition in Blender."
        )
    ...
    if args.gs_output_dir is None:
        ...
        if len(args.scene_path.split(sep)[-1]) > 0:
            gs_checkpoint_dir = os.path.join(
                "output", "vanilla_gs", args.scene_path.split(sep)[-1]
            )
        else:
            gs_checkpoint_dir = os.path.join(
                "output", "vanilla_gs", args.scene_path.split(sep)[-2]
            )
        ...
        safety_command = (
            ""  # TODO: Investigate why the MKL_SERVICE_FORCE_INTEL=1 flag is needed
        )
        os.system(
        ...
        print(
            "A vanilla 3DGS checkpoint was provided. Skipping the vanilla 3DGS optimization."
        )
    ...
    # Note `{depths_str}` is supported for sugar refinement but doesn't seem to help.
    os.system(
        f"python train.py \
            -s {args.scene_path} \
            -c {gs_checkpoint_dir} \
